...
export class TenantService {
  ...
  getTurfs(): Observable<TurfGround[]> {
    return this.http.get<TurfGround[]>(`${API_BASE}/turf/`, { headers: this.auth.authHeaders });
  }

  createTurf(turf: TurfGround): Observable<TurfGround> {
    return this.http.post<TurfGround>(`${API_BASE}/turf/`, turf, { headers: this.auth.authHeaders });
  }

  updateTurf(id: string, turf: Partial<TurfGround>): Observable<TurfGround> {
    return this.http.put<TurfGround>(`${API_BASE}/turf/${id}/`, turf, { headers: this.auth.authHeaders });
  }

  deleteTurf(id: string): Observable<any> {
    return this.http.delete(`${API_BASE}/turf/${id}/`, { headers: this.auth.authHeaders });
  }
}
